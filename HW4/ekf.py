...
class Ekf(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        self.Sigma = np.dot(np.dot(Gx, self.Sigma), Gx.T) + np.dot(np.dot(dt*Gu, self.R), Gu.T)
        ########## Code ends here ##########
...
#                innovation = angle_diff(z,h)
            ...
            if min_dist < self.g**2:
                v_list.append(min_innovation)
                Q_list.append(min_Q)
                H_list.append(min_Hs)
        ########## Code ends here ##########
...
class EkfSlam(Ekf):
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx_first_3 = tb.transform_line_to_scanner_frame(np.array([alpha, r]), self.x[0:3], self.tf_base_to_camera)
            
            x_b = self.tf_base_to_camera[0]
            y_b = self.tf_base_to_camera[1]
            x,y,th = self.x[0:3]
